/*********************************************************************
    ...
    Tiva LaunchPad (TM4C123GH6PM) Bare Matel Programming Demo
                      ...
                      BLINKY LED
     This Project uses Bit Band Region Programming to Blink
...
#define GPIO_PORTF_RED_LED           *(volatile uint32_t *)0x424A7F84
/*
       GPIOF_DATA_REG_BASE:         0x400253FC
       BIT BAND ALIAS ADDR:         0x42000000
       PERIPHERAL BASE ADDR:        0x40000000
       RED LED BIT NO:              1
       FORMULA: Bit Band Alias Region Base Addr + (byte offset from bit band region x 32 ) + (bit number x 4 )
             => 0x42000000 + ([0x400253FC-0x40000000] x 0x20) + (0x1 x 0x4)
       RED LED ADDR:                0x424A7F84
*/
...
int main (void) {
	...
	init_GPIOF();
	while (1) {
		GPIO_PORTF_RED_LED = 1;
		delay(5000);
		GPIO_PORTF_RED_LED = 0;
		delay(5000);
	}
