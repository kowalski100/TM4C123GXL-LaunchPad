/*********************************************************************
 ...
 This porject reads input from switch-1 and switch-2 and appropriatly
    turn ON/OFF the LEDs on Tiva LaunchPad using Negative Logic.
*********************************************************************/
...
int main (void) {
	...
	init_GPIOF();
	while (1) {
    port_data = GPIO_PORTF_DATA_REG&0x011;
    if ((port_data & 0x1)) {
      port_data &= ~(0x2UL);
    }else {
      port_data |= 0x2UL;
